...
router.post('/login', (req, res) => {
    ...
    if (!username || !password) {
        return res.status(400).send({ error: 'Campos obrigatórios faltando: username, password' });
    }
...
router.post('/admin', (req, res) => { 
    ...
    // Add validation for required fields
    if (!username || !password) {
        return res.status(400).render('admin-login-form', { error: 'Campos obrigatórios faltando: username, password', title: 'Login ADM Backend' });
    }
