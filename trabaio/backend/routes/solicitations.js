...
router.post('/', verifyJWT, (req, res) => {
  ...
  if (!pet || !servico || !data_hora || !status) {
    return res.status(400).send({ error: 'Campos obrigatórios faltando: pet, servico, data_hora, status' });
  }
...
router.put('/:id', (req, res) => {
  ...
  // Add validation for required fields
  if (!tutor || !pet || !servico || !data_hora || !status) {
    return res.status(400).json({ error: 'Campos obrigatórios faltando: tutor, pet, servico, data_hora, status' });
  }
