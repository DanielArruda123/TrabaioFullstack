...
router.post('/', verifyJWT, (req, res) => {
  ...
  if (!name || !race || !colour || !gender) {
    return res.status(400).send({ error: 'Campos obrigatórios faltando: name, race, colour, gender' });
  }
...
router.put('/:id', (req, res) => {
  ...
  // Add validation for required fields
  if (!name || !race || !colour || !gender) {
    return res.status(400).json({ error: 'Campos obrigatórios faltando: name, race, colour, gender' });
  }
