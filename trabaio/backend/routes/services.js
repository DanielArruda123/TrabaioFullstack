...
router.post('/', verifyAdmin, (req, res) => {
  ...
  if (!nome || !descricao || preco === undefined) {
    return res.status(400).send({ error: 'Campos obrigatórios faltando: nome, descricao, preco' });
  }
...
router.put('/:id', verifyAdmin, (req, res) => {
  ...
  // Add validation for required fields
  if (!nome || !descricao || preco === undefined) {
    return res.status(400).json({ error: 'Campos obrigatórios faltando: nome, descricao, preco' });
  }
